...
function getTicketNumber(params) {
    ...
    try {
        ...
        if (directTicketNum) {
            // Пробуем декодировать дважды, так как бот использует двойное кодирование
            try {
                const decodedNum = decodeURIComponent(decodeURIComponent(directTicketNum));
                console.log("Получен прямой номер билета (двойное декодирование):", decodedNum);
                return decodedNum.trim();
            } catch (decodingError) {
                // Если двойное декодирование не сработало, пробуем одиночное
                const decodedNum = decodeURIComponent(directTicketNum);
                console.log("Получен прямой номер билета (одиночное декодирование):", decodedNum);
                return decodedNum.trim();
            }
        }
    } catch (e) {
...
function initApp() {
    ...
    // Логируем для отладки, чтобы убедиться, что номер билета из бота правильно обрабатывается
    console.log("Исходный номер билета из бота:", ticketNumber);
    ...
    // Проверяем правильность формата номера билета (должен быть в формате "XXX XXX XXX")
    const ticketFormat = /^\d{3} \d{3} \d{3}$/;
    if (finalTicketNumber && !ticketFormat.test(finalTicketNumber)) {
        console.log("Номер билета не соответствует формату 'XXX XXX XXX', пробуем исправить формат");
        
        // Попытка исправить формат, если все цифры есть, но нет пробелов
        if (/^\d{9}$/.test(finalTicketNumber.replace(/\s/g, ''))) {
            const digits = finalTicketNumber.replace(/\s/g, '');
            finalTicketNumber = `${digits.substring(0, 3)} ${digits.substring(3, 6)} ${digits.substring(6, 9)}`;
            console.log("Исправленный формат номера билета:", finalTicketNumber);
        }
    }
    
    console.log("ИТОГОВЫЙ НОМЕР БИЛЕТА ПЕРЕД ОТОБРАЖЕНИЕМ:", finalTicketNumber);
    ...
    // Проверяем URL-параметры для частей номера билета (надежный способ передачи)
    const urlParams = new URLSearchParams(window.location.search);
    const ticketPart1 = urlParams.get('ticket_part1');
    const ticketPart2 = urlParams.get('ticket_part2');
    const ticketPart3 = urlParams.get('ticket_part3');
    
    // Если все части номера билета присутствуют, собираем их вместе
    if (ticketPart1 && ticketPart2 && ticketPart3) {
        const reconstructedTicket = `${ticketPart1} ${ticketPart2} ${ticketPart3}`;
        console.log("Реконструированный номер билета из частей:", reconstructedTicket);
        
        // Используем реконструированный номер с приоритетом
        if (reconstructedTicket.match(/^\d{3} \d{3} \d{3}$/)) {
            ticketData.ticket_number = reconstructedTicket;
            console.log("Установлен реконструированный номер билета:", ticketData.ticket_number);
        }
    }
    ...
    // Логируем финальный номер билета, который будет отображаться
