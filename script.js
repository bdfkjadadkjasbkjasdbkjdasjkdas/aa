...
function getTicketNumber(params) {
    if (params.ticket_number) {
        ...
        console.log("Получен номер билета из URL (используется):", ticketNum);
        return ticketNum;
    }
...
function initApp() {
    ...
    const ticketData = {
        ...
        // Приоритет прямого чтения из URL, затем из параметров, потом дефолт
        ticket_number: directTicketNumber || rawTicketNumber || ticketNumber,
    ...
    console.log("Обновляем интерфейс, используем номер билета:", ticketData.ticket_number);
