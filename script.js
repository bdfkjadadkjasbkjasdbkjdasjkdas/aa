...
function closeTicket() {
    ...
    // Ensure we're in the correct view state before closing
    // If we're in QR view, first go back to ticket view to prevent the flash
    if (isQRView) {
        // Force immediate transition by removing the animation
        const ticketContainer = document.getElementById('ticketContainer');
        ticketContainer.style.transition = 'none';
        showTicketView();
        
        // Small delay to ensure the DOM has updated before closing
        setTimeout(() => {
            try {
                tg.close();
            } catch (e) {
        ...
        // If already in ticket view, just close normally
        try {
            tg.close();
        } catch (e) {
            console.log("Ошибка при закрытии WebApp:", e);
