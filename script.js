...
function getTicketNumber(params) {
    ...
    // If all else fails, use default number
...
function closeTicket() {
    ...
    // If in QR view, switch to ticket view without animation before closing
    if (isQRView) {
        // Immediately hide ticket container to prevent any flashing
        const ticketContainer = document.getElementById('ticketContainer');
        
        // Disable all transitions
        ticketContainer.style.transition = 'none';
        
        // Force an immediate switch to ticket view (don't use showTicketView function
        // as it contains animations that might cause the flash)
        isQRView = false;
        ticketContainer.style.transform = 'translateX(0)';
        
        // Hide the entire container to prevent any flashing at all
