...
function getTicketNumber(params) {
    if (params.ticket_number) {
        return params.ticket_number;
    }
    // Генерация не требуется, так как номер всегда будет приходить из бота
    return "000 000 000";
}
...
function initApp() {
    ...
    // Преобразуем дату на русский язык, если она в другом формате
    if (ticketData.date && ticketData.date.includes('May') || ticketData.date.includes('June') || 
        ticketData.date.includes('April') || ticketData.date.includes('July')) {
        ...
        if (match) {
            ...
            const monthRus = monthsMapping[monthEng] || monthEng;
