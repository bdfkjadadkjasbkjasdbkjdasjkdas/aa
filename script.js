...
function initApp() {
    ...
    // Делаем двойную проверку наличия параметра ticket_number напрямую из URL
    const urlSearchParams = new URLSearchParams(window.location.search);
    const directTicketNumber = urlSearchParams.get('ticket_number');
    console.log("Прямое чтение номера билета из URL:", directTicketNumber);

    // Проверяем наличие параметра ticket_number напрямую из URL еще раз
    const urlSearchParams = new URLSearchParams(window.location.search);
    const directTicketNumber = urlSearchParams.get('ticket_number');
    console.log("Прямое чтение номера билета из URL (повторная проверка):", directTicketNumber);

    // Get ticket data from URL parameters with priority to direct URL params
    const ticketData = {
        carrier: urlSearchParams.get('carrier') || urlParams.carrier || 'ИП Патрин Н. Н.',
        route_number: urlSearchParams.get('route_number') || urlParams.route_number || '21',
        route_name: urlSearchParams.get('route_name') || urlParams.route_name || 'Парк "Прищепка" - Спортзал',
        bus_number: urlSearchParams.get('bus_number') || urlParams.bus_number || 'х312мв124',
        ticket_count: urlSearchParams.get('ticket_count') || urlParams.ticket_count || 1,
        // Высший приоритет у прямого параметра из URL
        ticket_number: directTicketNumber || ticketNumber,
        price: urlSearchParams.get('price') || urlParams.price || 44,
        date: urlSearchParams.get('date') || urlParams.date || currentDate,
        time: urlSearchParams.get('time') || urlParams.time || currentTime
    };
    ...
    document.getElementById('mainTicketNumber').textContent = window.finalTicketNumber;
    document.getElementById('qrTicketNum').textContent = window.finalTicketNumber;
    document.getElementById('qrNumberDisplay').textContent = `№ ${window.finalTicketNumber}`;
